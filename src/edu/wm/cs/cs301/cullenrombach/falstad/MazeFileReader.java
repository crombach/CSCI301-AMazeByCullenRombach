...
public class MazeFileReader {
	 ...
	 */
	public MazeFileReader(Activity activity, String filename) throws Exception{
	...
	public int getExpectedPartiters() {
	 ...
	 */
	private void load(Activity activity, String filename) throws Exception {
		try{
			// Load the content of the Maze xml file as a string.
			int chr;
			...
			InputSource is = new InputSource(new StringReader(data)); // From StackOverflow
			...
			// If there was a problem throw an error that lets the GeneratingActivity
			// know to build and save a new Maze.
			throw new Exception();
		}
	}
	...
	private static Seg readSegment(Element eElement, int number, int i) {
		...
		result.col = GraphicsWrapper.getColorArray(col); 
	...
	private static void compareSegments(ArrayList<Seg> slist, ArrayList<Seg> slist2) {
		...
		for (int i = 0 ; i < n ; i++)
		{
			...
			if(GraphicsWrapper.getRGB(s.col) != GraphicsWrapper.getRGB(s2.col)) 
