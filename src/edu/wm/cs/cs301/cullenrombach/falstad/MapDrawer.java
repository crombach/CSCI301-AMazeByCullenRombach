...
import edu.wm.cs.cs301.cullenrombach.falstad.GraphicsWrapper.Colors;
...
public class MapDrawer extends DefaultViewer {
	...
	@Override
	public void redraw(GraphicsWrapper gw, int state, int px, int py,
			int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		//dbg("redraw") ;
		if (state != Constants.STATE_PLAY)
			return ;
		if (maze.isInMapMode()) {
			// Set the line thickness to 2 pixels so they will show up properly.
			gw.setLineThickness(2);
			// Draw the graphics.
			draw_map(gw, px, py, walk_step, view_dx, view_dy, maze.isInShowMazeMode(), maze.isInShowSolutionMode()) ;
			draw_currentlocation(gw, view_dx, view_dy) ;
		}
	}
	/**
	 ...
	 * @param gw graphics handler to manipulate screen
	 */
	public void draw_map(GraphicsWrapper gw, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		gw.setColor(Colors.white);
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				gw.setColor(seencells.hasWallOnTop(x, y) ? Colors.white : Colors.gray);
				if ((seencells.hasWallOnTop(x, y) || showMaze) && w)
					gw.drawLine(nx1, ny1, nx2, ny1);
				...
				gw.setColor(seencells.hasWallOnLeft(x, y) ? Colors.white : Colors.gray);
				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					gw.drawLine(nx1, ny1, nx1, ny2);
			}
		if (showSolution) {
			draw_solution(gw, offx, offy, px, py) ;
		}
	 ...
	 * @param gw
	 */
	public void draw_currentlocation(GraphicsWrapper gw, int view_dx, int view_dy) {
		gw.setColor(Colors.red);
		// draw rectangle of appropriate size at the center of the screen
		int ctrx = view_width/2; // center x
		int ctry = view_height/2; // center y
		int cirsiz = map_scale/2; // circle size
		gw.fillRect(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		// draw a red arrow with the rectangle to indicate direction
		...
		gw.drawLine(ctrx, ctry, aptx, apty);
		gw.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		gw.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	 ...
	 * @param gw to draw lines on
	 * @param offx
	 * @param offy
	 */
	public void draw_solution(GraphicsWrapper gw, int offx, int offy, int px, int py) {
		// check parameters:
		if ((px < 0 || px >= mazew) || (py < 0 || py >= mazeh))
		{
		...
		while (d > 1) {
			...
			// do the graphics
			gw.drawLine(nx1, ny1, nx1+ndx, ny1+ndy);
